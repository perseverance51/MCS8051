...
PWM频率 = MAIN_Fosc / PWM_DUTY, 假设 MAIN_Fosc = 24MHZ, PWM_DUTY = 8000, 则输出PWM频率为2000HZ.
...
void	PCA_Init(void)
{
	...
	pwm0 = (PWM_DUTY / 4 * 1);	//给PWM一个初值2000
	pwm1 = (PWM_DUTY / 4 * 2);
	pwm2 = (PWM_DUTY / 4 * 3);
...
void	PCA_Handler (void) interrupt 7
{
	if(CCF0)		//PCA模块0中断
	{
		...
		CCAP0L = (u8)CCAP0_tmp;			//将影射寄存器写入捕获寄存器，先写CCAP0L低8位
		CCAP0H = (u8)(CCAP0_tmp >> 8);	//后写CCAP0H高8位
	}
